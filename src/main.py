...
def build_roi(config: Dict[str, Any]) -> DoorROI:
    ...
    return DoorROI(top_left, bottom_right, roi_cfg.get("direction"))
...
def run_pipeline(config: Dict[str, Any]) -> None:
    ...
    threshold_cfg = config.get("threshold", {"value": 30})
    background_cfg = config.get("background", {"learning_rate": 0.02})
    morph_cfg = config.get("morphology", {"erosion_iterations": 1, "dilation_iterations": 1})
    blob_cfg = config.get(
        "blob",
        {
            "min_area": 500,
    ...
    while True:
        ...
        binary_mask = binary_threshold(foreground, threshold_cfg.get("value", 30))
